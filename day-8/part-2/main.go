...
func main() {
	...
	for i := range distances {
		...
		if first.CircuitID == -1 && second.CircuitID == -1 {
			...
			////// check if that was the last junction
			if len(circuits[first.CircuitID]) == len(junctions) {
				fmt.Println(int(first.x) * int(second.x))
				break
			}
			...
			////// check if that was the last junction
			if len(circuits[second.CircuitID]) == len(junctions) {
				fmt.Println(int(first.x) * int(second.x))
				break
			}
			...
			////// check if that was the last junction
			if len(circuits[first.CircuitID])+len(circuits[second.CircuitID]) == len(junctions) {
				fmt.Println(int(first.x) * int(second.x))
				break
			}
